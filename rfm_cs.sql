...
-- Revenue and percentage of total revenue by customer segment 
SELECT 
    customer_segment,
    SUM(monetary_value) AS total_revenue,
    ROUND(SUM(monetary_value) / (SELECT SUM(TransactionAmount) FROM rfm_data) * 100, 1) AS pert_total_revenue
FROM customer_segmentation
GROUP BY customer_segment
ORDER BY total_revenue DESC;
-- Loyal Customers generate the highest total revenue of any segment ($167.3K, 32.6%),

-- Average recency (days since last purchase) by customer segment 
SELECT 
    customer_segment,
    COUNT(customer_id) AS number_of_customers,
...
-- At-risk Customers (210) show the second-highest recency gap at approximately 49.6 days,
-- representing $142.8K (27.8%) of revenue and a clear retention priority.
